...
void loop ()
{
  ...
  analogWrite(Pwm_Dr,150);// vitesse du moteur du  droite (vitesse pour qui tourne a droite) 


  Wire.beginTransmission(Moteur_ad);// adress du moteurdu gauche
  Wire.write(4);// sens du moteur (avant)
  Wire.endTransmission();// fin de transmission 
  analogWrite(Pwm_Ga,250);// vitesse du moteur gauche ( vitesse pour aui tourne a gauche)
}
